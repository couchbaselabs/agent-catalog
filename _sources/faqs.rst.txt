...
When a :py:class:`agentc.span.Span` instance is used, logs are continuously inserted into your Couchbase cluster.
...
The former holds all logs captured by :py:class:`agentc.span.Span` instances` in the :sql:`agent_activity.logs`
collection (see `here <analysis.html>`__ for more information).
...
How can I access the package-level logs generated by Agent Catalog?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Package-level logging (i.e., not logs captured by :py:class:`agentc.span.Span` instances) in Agent Catalog is handled by
Python's :python:`logging` package.
To see all package-level logs in the console while you run your application, set the ``AGENT_CATALOG_DEBUG``
variable to true:
...
This variable can also be set in your :file:`.env` file or as an argument when you construct your
:py:class:`agentc.catalog.Catalog` instance (e.g., :python:`agentc.catalog.Catalog(debug=True)` ).

To save all package level logs to a file, you would add a :python:`logging.FileHandler` to the :python:`agentc_*`
logger before any :python:`agentc.catalog.Catalog` calls occur (typically in the entrypoint of your application).
For example, logging all :python:`agentc_core` debug-level package logs to a file :file:`agentc.log` for the
:code:`with_langgraph` example can be realized by adding the following lines to the top of the :file:`main.py` file:

.. code-block:: python

       import logging

       debug_file_handler = logging.FileHandler(filename="agentc.log")
       debug_file_handler.setLevel(logging.DEBUG)

       logging.getLogger("agentc_core").addHandler(debug_file_handler)
       logging.getLogger("agentc_core").setLevel(logging.DEBUG)

.. tip::

  The same code can be applied to all Python package that use the :python:`logging` package!
  For more fine-grained control over logging in general (e.g., using a different format), refer to the
  :python:`logging` documentation `here <https://docs.python.org/3/howto/logging.html>`__.

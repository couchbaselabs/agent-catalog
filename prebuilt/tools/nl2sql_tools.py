...
@agentc.tool
def iq_tool(
    bucket, collection, scope, username, password, cluster_url, jwt_token, capella_address, org_id, natural_query
):
    ...
    def extract_schema(bucket, collection, scope, username, password, cluster_url) -> dict:
        ...
        try:
            ...
            result = cluster.query(f"INFER `{bucket}`.`{scope}`.`{collection}`;")
            for row in result:
                inferred_schema = row
...
if __name__ == "__main__":
    ...
    data = iq_tool(
        ...
        "https://api.dev.nonprod-project-avengers.com",
        "6af08c0a-8cab-4c1c-b257-b521575c16d0",

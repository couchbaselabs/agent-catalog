...
_KV_REGEX = fr'(({_KEY_REGEX}\s*=\s*{_VALUE_REGEX})|({_VALUE_REGEX}\s*=\s*{_KEY_REGEX}))'
_AND_REGEX = r'AND|and'
_OR_REGEX = r'OR|or'
_OP_REGEX = fr'(?P<op>{_AND_REGEX}|{_OR_REGEX})'
_QUERY_REGEX = fr'^\s*{_KV_REGEX}\s*({_OP_REGEX}\s*{_KV_REGEX}\s*)*$'
...
class AnnotationPredicate:
    def __init__(self, query: str):
        ...
        if matches is None:
            raise ValueError('Invalid query string for annotations! '
    ...
    def disjuncts(self) -> list[dict[str, str]]:
        if any(x == 'OR' for x in self.operators):
            ...
            working_partition: dict[str, str] = {
                next(key_iterator): next(value_iterator)
            }

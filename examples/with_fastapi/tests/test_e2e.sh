...
# End-to-end runner for examples/with_fastapi.
# Steps:
...
# 4) start FastAPI app and verify OpenAPI endpoint

set -euo pipefail

EXAMPLE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CB_CONTAINER="agentc-e2e-cb-fastapi-$$"
SERVER_PID=""
GIT_BACKUP_DIR=""
GIT_CREATED=0
...
run_app_cmd fastapi run server.py --workers 1 --host 127.0.0.1 --port 8000 > "$EXAMPLE_DIR/.e2e_fastapi.log" 2>&1 &
SERVER_PID=$!

log "Waiting for FastAPI OpenAPI endpoint"
wait_http "http://127.0.0.1:8000/openapi.json" 180

log "E2E success: FastAPI app is up"
log "Server log: $EXAMPLE_DIR/.e2e_fastapi.log"

...
# End-to-end runner for examples/with_langgraph.
# Steps:
...
# 4) run python main.py with scripted stdin and verify process output

set -euo pipefail

EXAMPLE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CB_CONTAINER="agentc-e2e-cb-langgraph-$$"
GIT_BACKUP_DIR=""
GIT_CREATED=0
...
run_app_cmd agentc init --add-hook-for tools --add-hook-for prompts

log "Creating initial commit"
git add -A
git commit --no-gpg-sign -m "Initial commit (e2e)" >/dev/null

log "Running langgraph app"
python3 - <<'PY'
import subprocess
import sys

import os
use_poetry_run = os.environ.get("USE_POETRY_RUN", "0") == "1"
cmd = ["python", "main.py"]
if use_poetry_run:
    cmd = ["poetry", "run", "python", "main.py"]
stdin_data = "I need to go to LAX from SFO\nthat's all, thanks\n"
...
try:
    ...
    with open(".e2e_langgraph.log", "w", encoding="utf-8") as fp:
        fp.write((exc.stdout or "") + "\n" + (exc.stderr or ""))
    print("LangGraph run timed out; see .e2e_langgraph.log", file=sys.stderr)
    sys.exit(1)
...
if result.returncode != 0:
    print("LangGraph run failed; see .e2e_langgraph.log", file=sys.stderr)
    sys.exit(result.returncode)

print("LangGraph run completed")
PY

log "E2E success: LangGraph app completed"
log "Run log: $EXAMPLE_DIR/.e2e_langgraph.log"

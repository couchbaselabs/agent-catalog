...
@main.command()
@click.option('-sm', '--sentence-models', multiple=True, default=['sentence-transformers/all-MiniLM-L12-v2'],
              help='SBERT models to download ahead of time.')
@click.option('-od', '--output-directory', default=DEFAULT_OUTPUT_DIRECTORY,
...
def init(sentence_models, output_directory, history_directory):
    ...
    cmd_initialize_local(sentence_models=sentence_models, output_directory=output_directory,
...
@click.option('-em', '--embedding-model', default='sentence-transformers/all-MiniLM-L12-v2',
